...
public class TouchButton {
    ...
    void initButton(final RPCView aRPCView, final int aPositionX, final int aPositionY, final int aWidthX, final int aHeightY,
                    final int aButtonColor, final String aText, final int aTextSizeAndFlags, final int aValue,
                    final int aCallbackAddress) {
        initButton(aRPCView, aPositionX, aPositionY, aWidthX, aHeightY, aButtonColor, aText, aTextSizeAndFlags & 0xFF,
                aTextSizeAndFlags >> 8, aValue, aCallbackAddress);
    }
        ...
        if (mTextSize > 0 && !mEscapedText.isEmpty()) { // don't render anything if text size == 0 or text is empty
            if (mTextStrings.length > 1) {
                ...
                if (mTextSize * mTextStrings.length >= mHeight) {
                    // Font height to big
                    MyLog.w(LOG_TAG, "text\"" + mEscapedText + "\" with " + mTextStrings.length + " lines to high");
                ...
                if (tLength >= mWidth) {
                    ...
                    MyLog.w(LOG_TAG, "text\"" + mEscapedText + "\" to long");
                } else {
                ...
                if (mTextSize >= mHeight) {
                    // Font height to big
                    MyLog.w(LOG_TAG, "text\"" + mEscapedText + "\" to high");
                }
            ...
            if (mTextStrings.length == 1) {
                ...
                for (String mTextString : mTextStrings) {
                    ...
                    if (tLength >= mWidth) {
                        ...
                        MyLog.w(LOG_TAG, "sub text\"" + mTextString + "\" to long");
                    } else {
    ...
    void setPosition(int aPositionX, int aPositionY) {
        ...
        if (aPositionX + mWidth > mRPCView.mRequestedCanvasWidth) {
            MyLog.e(LOG_TAG, mEscapedText + " Button x-position/width " + aPositionX + "+" + mWidth + " wrong. Set width to:"
        ...
        if (aPositionY + mHeight > mRPCView.mRequestedCanvasHeight) {
            MyLog.e(LOG_TAG, mEscapedText + " Button y-position/height " + aPositionY + "+" + mHeight + " wrong. Set height to:"
    ...
    public static void interpretCommand(final RPCView aRPCView, int aCommand, int[] aParameters, int aParamsLength,
        ...
        if (aCommand != FUNCTION_BUTTON_ACTIVATE_ALL && aCommand != FUNCTION_BUTTON_DEACTIVATE_ALL
            ...
            if (aParamsLength <= 0) {
                ...
                if (tButtonNumber >= 0 && tButtonNumber < sButtonList.size()) {
                    ...
                    if (MyLog.isINFO()) {
                        tButtonText = " \"" + tButton.mEscapedText + "\". ButtonNr=";
                    }
        ...
        switch (aCommand) {
            ...
            case FUNCTION_BUTTON_DRAW:
                if (MyLog.isINFO()) {
                    MyLog.i(LOG_TAG, "Draw button" + tButtonText + tButtonNumber);
                }
                tButton.drawButton();
                break;
            ...
            case FUNCTION_BUTTON_DRAW_TEXT:
                if (MyLog.isINFO()) {
                    MyLog.i(LOG_TAG, "Draw text=" + tButtonText + tButtonNumber);
                }
                tButton.drawText();
                break;
            ...
            case FUNCTION_BUTTON_SET_TEXT_AND_DRAW_BUTTON:
                ...
                if (MyLog.isINFO()) {
                    MyLog.i(LOG_TAG, "Set text=\"" + tButton.mEscapedText + "\" for" + tButtonText + tButtonNumber);
                }
            ...
            case FUNCTION_BUTTON_SET_TEXT_FOR_VALUE_TRUE:
                ...
                if (MyLog.isINFO()) {
                    MyLog.i(LOG_TAG, "Set text=\"" + tEscapedText + "\" for value true for" + tButtonText + tButtonNumber);
                }
                break;
            ...
            case FUNCTION_BUTTON_SETTINGS:
                ...
                switch (tSubcommand) {
                    ...
                    case SUBFUNCTION_BUTTON_SET_VALUE_AND_DRAW:
                        ...
                        if (MyLog.isINFO()) {
                            MyLog.i(LOG_TAG, "Set value=" + aParameters[2] + " for" + tButtonText + tButtonNumber);
                        }
                    ...
                    case SUBFUNCTION_BUTTON_SET_ACTIVE:
                        if (MyLog.isINFO()) {
                            MyLog.i(LOG_TAG, "Set active=true for" + tButtonText + tButtonNumber);
                        }
                        tButton.mIsActive = true;
                        break;
                    case SUBFUNCTION_BUTTON_RESET_ACTIVE:
                        if (MyLog.isINFO()) {
                            MyLog.i(LOG_TAG, "Set active=false for" + tButtonText + tButtonNumber);
                        }
                        tButton.mIsActive = false;
                        break;
            ...
            case FUNCTION_BUTTON_INIT:
                ...
                if (aParamsLength == 9) {
                    ...
                    tButton.initButton(aRPCView, aParameters[1], aParameters[2], aParameters[3], aParameters[4],
                            RPCView.shortToLongColor(aParameters[5]), tButtonText, aParameters[6], aParameters[7], tCallbackAddress);
                    if (MyLog.isINFO()) {
                        MyLog.i(LOG_TAG,
                                "Create button. ButtonNr=" + tButtonNumber + ", new TouchButton(\"" + tButton.mEscapedText
