...
#define COLOR_BLACK     ((Color_t)0X0001) // 16 because 0 is used as flag (e.g. in touch button for default color)
...
public:
    ...
    uint16_t drawChar(uint16_t aPosX, uint16_t aPosY, char aChar, uint8_t aCharSize, Color_t aFGColor, Color_t aBGColor);
    ...
    void setPrintfPositionColumnLine(uint8_t aColumnNumber, uint8_t aLineNumber);
    ...
    void drawChartByteBuffer(uint16_t aXOffset, uint16_t aYOffset, Color_t aColor, Color_t aClearBeforeColor, uint8_t aChartIndex,
            bool aDoDrawDirect, uint8_t *aByteBuffer, uint16_t aByteBufferLength);
