        ...
        public ConnectThread(BluetoothDevice device) {
            ...
            try {
                if (mAllowInsecureConnections) {
                    ...
                    tSocket = device.createInsecureRfcommSocketToServiceRecord(SerialPortServiceClass_UUID);
                } else {
                    tSocketType = "secure";
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                if (MyLog.isVERBOSE()) {
                    Log.v(LOG_TAG, "Start run - caught IOException - assume disconnected");
                }
            ...
            while (true) {
                try {
                     ...
                     */
                    tReadLength = mmInStream.read(mSerialService.mBigReceiveBuffer, mSerialService.mReceiveBufferInIndex,
                            BT_READ_MAX_SIZE);
                    ...
                    // before
                    if (MyLog.isINFO()) {
                        MyLog.i(LOG_TAG, "Caught IOException - assume disconnected");
                    }
                    connectionLost(); // show toast
                    break;
                }
